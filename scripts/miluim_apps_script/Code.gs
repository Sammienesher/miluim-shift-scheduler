...
// Convert YYYY-MM-DD → integer YYYYMMDD for reliable numeric comparison
...
function getShifts(personName, startDate, endDate) {
  ...
  // Convert to numeric YYYYMMDD for rock-solid comparison
  ...
  for (let i = 0; i < numCols; i++) {
    ...
    const isoDate = ddmmyyToISO(match[1], match[2], match[3]);
    const dateNum = dateToNum(isoDate);

    // Numeric comparison — immune to timezone, string encoding, etc.
